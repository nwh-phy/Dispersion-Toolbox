...
switch opts.tracking_mode
    ...
    case 'windowed_branch_tracking'
        result = local_extract_windowed(unit_data, energy_axis, ...
            energy_window, opts);
    case 'ridge_guided_candidate_path'
        result = local_extract_ridge_guided(unit_data, energy_axis, ...
            energy_window, opts);
    otherwise
...
function result = local_extract_ridge_guided(unit_data, energy_axis, ...
    energy_window, opts)
result = local_empty_extraction_result(numel(unit_data));
[candidate_points, candidate_outcomes, unit_failures] = ...
    local_ridge_guided_candidates(unit_data, energy_axis, energy_window, opts);
result.candidate_points = candidate_points;
if isempty(candidate_points) || height(candidate_points) == 0
    for ui = 1:numel(unit_failures)
        result = local_append_outcome(result, ui, unit_failures{ui});
    end
    result = local_sort_extraction_result(result);
    return
end

selection = b1_double_peak_candidate_path_select(candidate_points, ...
    sessionKey=opts.session_key, ...
    upperMediumJumpThresholdMeV=opts.candidate_path_upper_medium_jump_threshold_meV, ...
    largeJumpThresholdMeV=opts.candidate_path_large_jump_threshold_meV, ...
    edgeEnergyMeV=opts.candidate_path_edge_energy_meV, ...
    upperMaxGammaOverE=opts.upper_max_gamma_over_E, ...
    upperMaxGammaMeV=opts.upper_max_gamma_meV, ...
    minPeakSeparationMeV=opts.min_peak_separation_meV, ...
    upperTrendMode=opts.candidate_path_upper_trend_mode, ...
    upperTrendAnchorQAbsAinv=opts.candidate_path_upper_trend_anchor_q_abs_Ainv, ...
    upperTrendSmallQAbsAinv=opts.candidate_path_upper_trend_small_q_abs_Ainv, ...
    upperTrendPlateauQAbsAinv=opts.candidate_path_upper_trend_plateau_q_abs_Ainv);
result.path_selection = selection.path_selection;
selected_ids = selection.path_selection.selected_candidate_id;
selected_ui = false(numel(unit_data), 1);
for i = 1:numel(selected_ids)
    id = selected_ids(i);
    if id >= 1 && id <= numel(candidate_outcomes) && ...
            ~isempty(candidate_outcomes{id})
        outcome = candidate_outcomes{id};
        ui = outcome.unit_index;
        selected_ui(ui) = true;
        result = local_append_outcome(result, ui, outcome);
    end
end
...
function [candidate_points, candidate_outcomes, unit_failures] = ...
    local_ridge_guided_candidates(unit_data, energy_axis, energy_window, opts)
candidate_points = local_empty_candidate_points_table();
candidate_outcomes = {};
unit_failures = cell(numel(unit_data), 1);
if isempty(unit_data)
    return
end

q_values = arrayfun(@(x) x.unit.q_Ainv, unit_data(:));
fit_matrix = zeros(numel(energy_axis), numel(unit_data));
for ui = 1:numel(unit_data)
    fit_matrix(:, ui) = double(unit_data(ui).fit_spectrum(:));
end
ridge = b1_double_peak_ridge_reference(energy_axis, q_values(:), ...
    fit_matrix, energyWindowMeV=energy_window, ...
    smoothWindow=opts.ridge_smooth_window, ...
    minSeparationMeV=opts.min_peak_separation_meV);

candidate_id = 0;
for ui = 1:numel(unit_data)
    data = unit_data(ui);
    guess_pairs = local_ridge_candidate_guess_pairs(data, ridge, ui, ...
        energy_axis, energy_window, opts);
    first_failure = [];
    local_success = false;
    for gi = 1:height(guess_pairs)
        guesses = [guess_pairs.lower_initial_guess_meV(gi); ...
            guess_pairs.upper_initial_guess_meV(gi)];
        outcome = local_fit_double_unit(data, energy_axis, energy_window, ...
            opts, guesses);
        if ~outcome.success
        ...
        outcome.unit_index = ui;
        outcome.candidate_source = guess_pairs.candidate_source{gi};
        candidate_outcomes{candidate_id, 1} = outcome; %#ok<AGROW>
        candidate_points = [candidate_points; local_candidate_row( ...
            candidate_id, data.unit, outcome, guess_pairs(gi, :), opts)]; %#ok<AGROW>
        local_success = true;
    end
    if ~local_success
        if isempty(first_failure)
            first_failure = local_failed_double_outcome(data.unit, ...
                data.guesses, 'ridge_guided_no_candidate', ...
                'no ridge-guided candidate guesses were available', ...
                data.fit_meta);
        end
        unit_failures{ui} = first_failure;
    else
        unit_failures{ui} = local_failed_double_outcome(data.unit, ...
            data.guesses, 'ridge_guided_candidate_not_selected', ...
            'unit had candidates but was not selected by path selection', ...
            data.fit_meta);
    end
end
end
...
function pairs = local_ridge_candidate_guess_pairs(data, ridge, ui, ...
    energy_axis, energy_window, opts)
pairs = local_empty_candidate_guess_table();
ridge_lower = ridge.lower_points.energy_meV(ui);
ridge_upper = ridge.upper_points.energy_meV(ui);
pairs = local_add_candidate_pair(pairs, 'ridge_window', ridge_lower, ...
    ridge_upper, energy_window, opts);

if numel(data.guesses) >= 2
    pairs = local_add_candidate_pair(pairs, 'local_peak_pair', ...
        data.guesses(1), data.guesses(2), energy_window, opts);
end

center = mean(data.guesses(isfinite(data.guesses)), 'omitnan');
if ~isfinite(center)
    mask = energy_axis >= energy_window(1) & energy_axis <= energy_window(2);
    [~, idx] = max(data.fit_spectrum(mask), [], 'omitnan');
    ewin = energy_axis(mask);
    if ~isempty(idx)
        center = ewin(idx);
    end
end
if ~isfinite(center)
    center = mean(energy_window);
end
for split = opts.fallback_split_candidates_meV(:).'
    pairs = local_add_candidate_pair(pairs, 'split_pair', ...
        center - split / 2, center + split / 2, energy_window, opts);
end

if isfinite(ridge_upper)
    half_width = local_upper_retry_half_width(data.unit, opts);
    pairs = local_add_candidate_pair(pairs, 'upper_retry_pair', ...
        ridge_lower, min(ridge_upper + 0.25 * half_width, ...
        energy_window(2) - opts.initial_peak_margin_meV), ...
        energy_window, opts);
end
...
function pairs = local_add_candidate_pair(pairs, source, lower, upper, ...
    energy_window, opts)
if ~isfinite(lower) || ~isfinite(upper)
    return
end
values = sort([lower upper]);
margin = opts.initial_peak_margin_meV;
values(1) = min(max(values(1), energy_window(1) + margin), ...
    energy_window(2) - margin);
values(2) = min(max(values(2), energy_window(1) + margin), ...
    energy_window(2) - margin);
if values(2) - values(1) < opts.min_peak_separation_meV
    return
end
row = table({source}, values(1), values(2), ...
    'VariableNames', {'candidate_source', 'lower_initial_guess_meV', ...
    'upper_initial_guess_meV'});
pairs = [pairs; row];
end


function row = local_candidate_row(candidate_id, unit, outcome, pair, opts)
lower = outcome.lower_row;
upper = outcome.upper_row;
lower_gamma_over_E = lower.gamma_meV ./ max(abs(lower.energy_meV), eps);
upper_gamma_over_E = upper.gamma_meV ./ max(abs(upper.energy_meV), eps);
edge = sort(opts.candidate_path_edge_energy_meV);
lower_edge = lower.energy_meV < edge(1) | lower.energy_meV > edge(2);
upper_edge = upper.energy_meV < edge(1) | upper.energy_meV > edge(2);
upper_overbroad = upper_gamma_over_E > opts.upper_max_gamma_over_E | ...
    upper.gamma_meV > opts.upper_max_gamma_meV;
row = table(candidate_id, unit.q_Ainv, unit.q_abs_Ainv, ...
    pair.candidate_source, pair.lower_initial_guess_meV, ...
    pair.upper_initial_guess_meV, lower.energy_meV, upper.energy_meV, ...
    lower.gamma_meV, upper.gamma_meV, lower_gamma_over_E, ...
    upper_gamma_over_E, lower.R2, upper.R2, mean([lower.R2 upper.R2]), ...
    lower_edge, upper_edge, upper_overbroad, {unit.source_mode}, ...
    unit.source_q_count, {unit.source_q_Ainv}, {unit.source_q_index}, ...
    unit.bin_size_requested, lower.peak_model, lower.tracking_mode, ...
    lower.fit_spectrum_source, lower.fit_denoise_method, ...
    lower.fit_denoise_window, lower.fit_denoise_order, ...
    lower.fit_denoise_profile, lower.fit_denoise_q_start_Ainv, ...
    lower.fit_denoise_q_end_Ainv, lower.fit_spectrum_delta_rms, ...
    'VariableNames', {'candidate_id', 'q_Ainv', 'q_abs_Ainv', ...
    'candidate_source', 'lower_initial_guess_meV', ...
    'upper_initial_guess_meV', 'lower_energy_meV', ...
    'upper_energy_meV', 'lower_gamma_meV', 'upper_gamma_meV', ...
    'lower_gamma_over_E', 'upper_gamma_over_E', 'lower_R2', ...
    'upper_R2', 'R2', 'lower_edge_flag', 'upper_edge_flag', ...
    'upper_overbroad_flag', 'source_mode', 'source_q_count', ...
    'source_q_Ainv', 'source_q_index', 'bin_size_requested', ...
...
try
    fit = fit_loss_function(energy_axis, data.fit_spectrum, ...
        ...
        'min_peak_amplitude_fraction', opts.min_peak_amplitude_fraction);
...
result.candidate_points = local_empty_candidate_points_table();
result.path_selection = local_empty_path_selection_table();
...
opts = local_set_default(opts, 'min_peak_amplitude_fraction', 0.10);
...
opts = local_set_default(opts, 'session_key', '');
opts = local_set_default(opts, 'ridge_smooth_window', 9);
opts = local_set_default(opts, 'candidate_path_upper_medium_jump_threshold_meV', NaN);
opts = local_set_default(opts, 'candidate_path_large_jump_threshold_meV', 250);
opts = local_set_default(opts, 'candidate_path_edge_energy_meV', [650 1900]);
opts = local_set_default(opts, 'candidate_path_upper_trend_mode', 'none');
opts = local_set_default(opts, 'candidate_path_upper_trend_anchor_q_abs_Ainv', 0.005);
opts = local_set_default(opts, 'candidate_path_upper_trend_small_q_abs_Ainv', 0.02);
opts = local_set_default(opts, 'candidate_path_upper_trend_plateau_q_abs_Ainv', 0.06);
...
opts.min_peak_amplitude_fraction = max(0, double(opts.min_peak_amplitude_fraction));
...
opts.session_key = char(string(opts.session_key));
opts.ridge_smooth_window = local_odd_window(opts.ridge_smooth_window);
opts.candidate_path_upper_medium_jump_threshold_meV = ...
    double(opts.candidate_path_upper_medium_jump_threshold_meV);
opts.candidate_path_large_jump_threshold_meV = max(1, ...
    double(opts.candidate_path_large_jump_threshold_meV));
opts.candidate_path_edge_energy_meV = sort(double( ...
    opts.candidate_path_edge_energy_meV(:)).');
if numel(opts.candidate_path_edge_energy_meV) ~= 2
    opts.candidate_path_edge_energy_meV = [650 1900];
end
opts.candidate_path_upper_trend_mode = lower(strtrim(char(string( ...
    opts.candidate_path_upper_trend_mode))));
opts.candidate_path_upper_trend_anchor_q_abs_Ainv = max(0, double( ...
    opts.candidate_path_upper_trend_anchor_q_abs_Ainv));
opts.candidate_path_upper_trend_small_q_abs_Ainv = max( ...
    opts.candidate_path_upper_trend_anchor_q_abs_Ainv, double( ...
    opts.candidate_path_upper_trend_small_q_abs_Ainv));
opts.candidate_path_upper_trend_plateau_q_abs_Ainv = max( ...
    opts.candidate_path_upper_trend_small_q_abs_Ainv, double( ...
...
function tbl = local_empty_candidate_guess_table()
names = {'candidate_source', 'lower_initial_guess_meV', ...
    'upper_initial_guess_meV'};
types = {'cell', 'double', 'double'};
tbl = table('Size', [0 numel(names)], 'VariableTypes', types, ...
    'VariableNames', names);
end


function tbl = local_empty_candidate_points_table()
names = {'candidate_id', 'q_Ainv', 'q_abs_Ainv', 'candidate_source', ...
    'lower_initial_guess_meV', 'upper_initial_guess_meV', ...
    'lower_energy_meV', 'upper_energy_meV', 'lower_gamma_meV', ...
    'upper_gamma_meV', 'lower_gamma_over_E', 'upper_gamma_over_E', ...
    'lower_R2', 'upper_R2', 'R2', 'lower_edge_flag', ...
    'upper_edge_flag', 'upper_overbroad_flag', 'source_mode', ...
    'source_q_count', 'source_q_Ainv', 'source_q_index', ...
...
types = {'double', 'double', 'double', 'cell', 'double', 'double', ...
    ...
    'cell', 'double', 'cell', 'cell', 'double', 'cell', 'cell', ...
...
function tbl = local_empty_path_selection_table()
