...
%         .on_fit_param_change, .on_correction_target_change,
...
%   callbacks but does NOT read or modify application state.
    ...
    add_label(control_grid, 3, 12, "Dn profile");
    denoise_profile = uidropdown(control_grid, ...
        "Items", {'Global', 'Adaptive |q|'}, ...
        "Value", 'Global', ...
        "Tooltip", "Global uses one denoise strength. Adaptive |q| uses one method with smoothly increasing strength at larger |q|.", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_profile.Layout.Row = 3;
    denoise_profile.Layout.Column = 13;

    add_label(control_grid, 3, 14, "q0 / width");
    denoise_q_split = uieditfield(control_grid, "numeric", ...
        "Value", 0.040, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.5f", ...
        "Tooltip", "Adaptive |q| transition center in A^-1.", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_q_split.Layout.Row = 3;
    denoise_q_split.Layout.Column = 15;

    denoise_ramp_width = uieditfield(control_grid, "numeric", ...
        "Value", 0.020, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.5f", ...
        "Tooltip", "Adaptive |q|: transition half-width in A^-1. Larger values make the strength change more gradual.", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_ramp_width.Layout.Row = 3;
    denoise_ramp_width.Layout.Column = 16;

    denoise_lowq_method = uidropdown(control_grid, ...
        "Items", {'Low Wiener2D', 'Low SavGol', 'Low BM3D'}, ...
        "ItemsData", {'Wiener2D', 'SavGol', 'BM3D'}, ...
        "Value", 'Wiener2D', ...
        "Tooltip", "Legacy split-q low-q method.", ...
        "Visible", "off", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_lowq_method.Layout.Row = 4;
    denoise_lowq_method.Layout.Column = 15;

    denoise_highq_method = uidropdown(control_grid, ...
        "Items", {'High Wiener2D', 'High SavGol', 'High BM3D'}, ...
        "ItemsData", {'Wiener2D', 'SavGol', 'BM3D'}, ...
        "Value", 'BM3D', ...
        "Tooltip", "Legacy split-q high-q method.", ...
        "Visible", "off", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_highq_method.Layout.Row = 4;
    denoise_highq_method.Layout.Column = 16;

    denoise_low_strength = uieditfield(control_grid, "numeric", ...
        "Value", 0.30, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.3g", ...
        "Tooltip", "Adaptive |q| low-q strength. For BM3D this is BMfactor.", ...
        "Visible", "off", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_low_strength.Layout.Row = 4;
    denoise_low_strength.Layout.Column = 15;

    denoise_high_strength = uieditfield(control_grid, "numeric", ...
        "Value", 0.80, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.3g", ...
        "Tooltip", "Adaptive |q| high-q strength. For BM3D this is BMfactor.", ...
        "Visible", "off", ...
        "ValueChangedFcn", cb.on_display_change);
    denoise_high_strength.Layout.Row = 4;
    denoise_high_strength.Layout.Column = 16;
    ...
    add_label(control_grid, 6, 3, "σ / BM f");
    denoise_sigma = uieditfield(control_grid, "numeric", ...
        "Value", 0, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.1f", ...
        "Tooltip", "Wiener2D: 0 = auto-estimate noise. BM3D: 0 = weak default factor 0.5; use 0.2-0.8 for weaker/stronger BM3D.", ...
        "ValueChangedFcn", cb.on_display_change);
    ...
    export_btn = uibutton(control_grid, ...
        "Text", "📷 Export", ...
        "Tooltip", "Export q-E map, comparison, single spectrum, and dispersion panels", ...
        "ButtonPushedFcn", cb.on_export);
    export_btn.Text = "Export Panels";
    export_btn.Layout.Row = 10;
    export_btn.Layout.Column = [11 12];

    auto_fit_btn = uibutton(control_grid, ...
        "Text", "Auto Fit Branches", ...
        "ButtonPushedFcn", cb.on_auto_fit_dispersion);
    auto_fit_btn.Layout.Row = 7;
    auto_fit_btn.Layout.Column = [1 2];

    branch1_lbl = uilabel(control_grid, "Text", "B1 meV");
    branch1_lbl.Layout.Row = 7; branch1_lbl.Layout.Column = 6;
    branch1_min = uieditfield(control_grid, "numeric", ...
        "Value", 500, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 1 lower energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch1_min.Layout.Row = 7; branch1_min.Layout.Column = 7;
    branch1_max = uieditfield(control_grid, "numeric", ...
        "Value", 2100, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 1 upper energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch1_max.Layout.Row = 7; branch1_max.Layout.Column = 8;

    branch2_lbl = uilabel(control_grid, "Text", "B2 meV");
    branch2_lbl.Layout.Row = 7; branch2_lbl.Layout.Column = 9;
    branch2_min = uieditfield(control_grid, "numeric", ...
        "Value", 1700, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 2 lower energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch2_min.Layout.Row = 7; branch2_min.Layout.Column = 10;
    branch2_max = uieditfield(control_grid, "numeric", ...
        "Value", 2600, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 2 upper energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch2_max.Layout.Row = 7; branch2_max.Layout.Column = 11;

    branch3_lbl = uilabel(control_grid, "Text", "B3 meV");
    branch3_lbl.Layout.Row = 7; branch3_lbl.Layout.Column = 12;
    branch3_min = uieditfield(control_grid, "numeric", ...
        "Value", 2800, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 3 lower energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch3_min.Layout.Row = 7; branch3_min.Layout.Column = 13;
    branch3_max = uieditfield(control_grid, "numeric", ...
        "Value", 3800, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Branch 3 upper energy window (meV)", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    branch3_max.Layout.Row = 7; branch3_max.Layout.Column = 14;

    pts_label = uilabel(control_grid, ...
        "Text", "0 pts", ...
        "HorizontalAlignment", "left");
    pts_label.Layout.Row = 7;
    pts_label.Layout.Column = 15;

    export_ratio = uidropdown(control_grid, ...
        "Items", {'1:1', '4:3', '3:2', '16:9', '2:1'}, ...
        "Value", '16:9', ...
        "Tooltip", "Export panel aspect ratio", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    export_ratio.Layout.Row = 10;
    export_ratio.Layout.Column = 15;

    export_mode = uidropdown(control_grid, ...
        "Items", {'Quick PNG', 'PPT PNG', 'Publication'}, ...
        "Value", 'Quick PNG', ...
        "Tooltip", "Quick PNG: 150 DPI PNG only; PPT PNG: 300 DPI PNG only; Publication: 300 DPI PNG + PDF", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    export_mode.Layout.Row = 10;
    export_mode.Layout.Column = [13 14];
    ...
    fit_info_label.Layout.Column = [13 16];
    ...
    pk_model_dropdown = uidropdown(control_grid, ...
        "Items", {'Lorentz', 'Gaussian', 'Voigt', 'Damped HO', 'Fano'}, ...
        "ItemsData", {'lorentz', 'gaussian', 'voigt', 'damped_ho', 'fano'}, ...
        "Value", 'lorentz', ...
        "ValueChangedFcn", cb.on_fit_param_change);
    ...
    correct_auto_btn.Layout.Row = 9; correct_auto_btn.Layout.Column = [6 7];
    ...
    undo_corr_btn.Layout.Row = 9; undo_corr_btn.Layout.Column = 8;
    ...
    corr_branch_dropdown = uidropdown(control_grid, ...
        ...
        "ValueChangedFcn", cb.on_correction_target_change);
    corr_branch_dropdown.Layout.Row = 9;
    corr_branch_dropdown.Layout.Column = [9 10];
    ...
    fit_branch_lbl = uilabel(control_grid, "Text", "Fit Branch");
    fit_branch_lbl.Layout.Row = 10; fit_branch_lbl.Layout.Column = 5;

    fit_branch_dropdown = uidropdown(control_grid, ...
        "Items", {'All'}, ...
        "Value", 'All', ...
        "Tooltip", "Choose which curated branch to fit", ...
        "ValueChangedFcn", cb.on_fit_param_change);
    fit_branch_dropdown.Layout.Row = 10;
    fit_branch_dropdown.Layout.Column = [6 7];

    fit_disp_btn = uibutton(control_grid, ...
        "Text", "Fit Curated", ...
        "ButtonPushedFcn", cb.on_fit_dispersion);
    fit_disp_btn.Layout.Row = 10; fit_disp_btn.Layout.Column = [8 9];

    export_disp_btn = uibutton(control_grid, ...
        "Text", "Export Disp.", ...
        "ButtonPushedFcn", cb.on_export_dispersion);
    export_disp_btn.Layout.Row = 10; export_disp_btn.Layout.Column = 10;
    ...
    disp_info_label.Layout.Column = 16;
    ...
    title(comparison_axes, "Comparison Off-axis Component");
    ...
    waterfall_grid = uigridlayout(waterfall_tab, [2 1]);
    waterfall_grid.RowHeight = {28, "1x"};
    waterfall_grid.Padding = [0 0 0 0];
    waterfall_grid.RowSpacing = 4;
    waterfall_grid.ColumnSpacing = 0;

    waterfall_controls = uigridlayout(waterfall_grid, [1 13]);
    waterfall_controls.Layout.Row = 1;
    waterfall_controls.Layout.Column = 1;
    waterfall_controls.ColumnWidth = {52, 58, 46, 58, 48, 74, 58, 58, 58, 58, 44, 58, "1x"};
    waterfall_controls.Padding = [4 2 4 0];
    waterfall_controls.RowSpacing = 0;
    waterfall_controls.ColumnSpacing = 4;

    add_label(waterfall_controls, 1, 1, "WF min");
    waterfall_min = uieditfield(waterfall_controls, "numeric", ...
        "Value", 50, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Stacked spectra display lower bound (meV). Does not change extraction.", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_min.Layout.Row = 1;
    waterfall_min.Layout.Column = 2;

    add_label(waterfall_controls, 1, 3, "WF max");
    waterfall_max = uieditfield(waterfall_controls, "numeric", ...
        "Value", 2000, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Stacked spectra display upper bound (meV). Does not change extraction.", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_max.Layout.Row = 1;
    waterfall_max.Layout.Column = 4;

    add_label(waterfall_controls, 1, 5, "Norm");
    waterfall_norm = uidropdown(waterfall_controls, ...
        "Items", {'none', 'area'}, ...
        "Value", 'area', ...
        "Tooltip", "Stacked spectra display normalization. Area divides each trace by its selected energy-window area.", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_norm.Layout.Row = 1;
    waterfall_norm.Layout.Column = 6;

    add_label(waterfall_controls, 1, 7, "N min");
    waterfall_norm_min = uieditfield(waterfall_controls, "numeric", ...
        "Value", 50, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Area-normalization lower bound (meV).", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_norm_min.Layout.Row = 1;
    waterfall_norm_min.Layout.Column = 8;

    add_label(waterfall_controls, 1, 9, "N max");
    waterfall_norm_max = uieditfield(waterfall_controls, "numeric", ...
        "Value", 3800, ...
        "ValueDisplayFormat", "%.0f", ...
        "Tooltip", "Area-normalization upper bound (meV).", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_norm_max.Layout.Row = 1;
    waterfall_norm_max.Layout.Column = 10;

    add_label(waterfall_controls, 1, 11, "Gain");
    waterfall_gain = uieditfield(waterfall_controls, "numeric", ...
        "Value", 1, ...
        "Limits", [0 inf], ...
        "ValueDisplayFormat", "%.3g", ...
        "Tooltip", "Display-only vertical multiplier before offsets.", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_gain.Layout.Row = 1;
    waterfall_gain.Layout.Column = 12;

    waterfall_residual = uicheckbox(waterfall_controls, ...
        "Text", "Residual", ...
        "Value", false, ...
        "Tooltip", "Display-only broad-background subtraction for stacked spectra.", ...
        "ValueChangedFcn", cb.on_display_change);
    waterfall_residual.Layout.Row = 1;
    waterfall_residual.Layout.Column = 13;

    waterfall_axes = uiaxes(waterfall_grid);
    waterfall_axes.Layout.Row = 2;
    ...
    ui_handles.WaterfallMinField = waterfall_min;
    ui_handles.WaterfallMaxField = waterfall_max;
    ui_handles.WaterfallNormDropdown = waterfall_norm;
    ui_handles.WaterfallNormMinField = waterfall_norm_min;
    ui_handles.WaterfallNormMaxField = waterfall_norm_max;
    ui_handles.WaterfallGainField = waterfall_gain;
    ui_handles.WaterfallResidualCheckbox = waterfall_residual;
    ...
    ui_handles.Branch1MinField = branch1_min;
    ui_handles.Branch1MaxField = branch1_max;
    ui_handles.Branch2MinField = branch2_min;
    ui_handles.Branch2MaxField = branch2_max;
    ui_handles.Branch3MinField = branch3_min;
    ui_handles.Branch3MaxField = branch3_max;
    ...
    ui_handles.ExportModeDropdown = export_mode;
    ...
    ui_handles.DenoiseProfileDropdown = denoise_profile;
    ui_handles.DenoiseQSplitField = denoise_q_split;
    ui_handles.DenoiseRampWidthField = denoise_ramp_width;
    ui_handles.DenoiseLowStrengthField = denoise_low_strength;
    ui_handles.DenoiseHighStrengthField = denoise_high_strength;
    ui_handles.DenoiseLowQMethodDropdown = denoise_lowq_method;
    ui_handles.DenoiseHighQMethodDropdown = denoise_highq_method;
    ...
    ui_handles.FitBranchDropdown = fit_branch_dropdown;
