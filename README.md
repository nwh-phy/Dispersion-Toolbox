...
```matlab
cd('path/to/Dispersion-Toolbox') % cd 到项目根目录
...
- **Denoise**：Wiener 2D 去噪 / Savitzky-Golay 滤波 / **BM3D**（块匹配 3D 协同滤波，来自 Nion toolbox）
- **Deconv**：Lucy-Richardson 迭代退卷积（ZLP 作为 PSF）
- **BG Sub**：背景扣除（详见下方）
...
├── toolbox_config.m.template    # 本地配置模板（按需复制为 toolbox_config.m）
│
├── src/                         # 所有 MATLAB 源码
...
│
└── lib/
    └── BM3D/                    # BM3D 去噪实现与预编译 mex 文件
```

> 说明：README 早期版本里出现过 `figures/`、`docs/`、`references/`、实验数据目录等示例路径，这些目录**不属于当前 Git 仓库**；如有需要请在本地自行创建并加入 `.gitignore` 管理。
