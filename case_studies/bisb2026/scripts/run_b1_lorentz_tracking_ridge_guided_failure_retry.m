function out = run_b1_lorentz_tracking_ridge_guided_failure_retry(options)
%RUN_B1_LORENTZ_TRACKING_RIDGE_GUIDED_FAILURE_RETRY Targeted weak-peak retry.
%
% This retry keeps the clean 300 meV start window, mandatory Lorentz
% double peaks, signed q, no +q/-q averaging, and no single-peak fallback.
% It lowers only the post-fit weak-peak amplitude filter so a real weak
% second Lorentz component is not discarded before scoring.
...
if ~strcmp(char(string(options.manualAnchorPolicy)), 'template_only')
    error('run_b1_lorentz_tracking_ridge_guided_failure_retry:ManualAnchors', ...
...
fprintf('\n=== B1 ridge-guided failure retry %s ===\n', variant.id);
if options.runExtraction
    local_run_variant(variant);
end
[metrics, upper_diag] = local_finalize_variant(project_root, summary_dir, variant);
...
metrics_path = fullfile(summary_dir, ...
    'b1_lorentz_tracking_ridge_guided_failure_retry_metrics.csv');
writetable(metrics, metrics_path);
upper_diag_path = fullfile(summary_dir, ...
    'b1_lorentz_tracking_ridge_guided_failure_retry_upper_jump_gamma_R2.csv');
writetable(upper_diag, upper_diag_path);
variant_path = fullfile(summary_dir, ...
    'b1_lorentz_tracking_ridge_guided_failure_retry_variants.csv');
...
if strlength(best.overlay_png) > 0 && isfile(best.overlay_png)
    copyfile(best.overlay_png, fullfile(summary_dir, ...
        'b1_double_peak_lorentz_tracking_failure_retry_overlay_three_dataset_comparison.png'));
end
if strlength(best.overlay_pdf) > 0 && isfile(best.overlay_pdf)
    copyfile(best.overlay_pdf, fullfile(summary_dir, ...
        'b1_double_peak_lorentz_tracking_failure_retry_overlay_three_dataset_comparison.pdf'));
end
...
readme_path = fullfile(summary_dir, ...
    'README_lorentz_tracking_ridge_guided_failure_retry.md');
local_write_readme(readme_path, metrics, best, options.targetScore, gate_passed);
local_register_outputs(project_root, summary_dir, best, gate_passed || score < 358);
...
fprintf('\nB1 ridge-guided failure retry complete.\n');
fprintf('  Summary: %s\n', summary_dir);
...
variant = struct();
variant.id = 'v14_failure_retry_amp0_split40_420';
variant.final_tag = '260510_lorentz_tracking_ridge_guided_v14_failure_retry_amp0_split40_420';
...
for si = 1:numel(sessions)
    ...
    writetable(rows, fullfile(session_dir, sprintf( ...
        'b1_double_peak_lorentz_tracking_ridge_guided_failure_retry_%s_tracking_metrics.csv', ...
        variant.id)));
    writetable(diag_rows, fullfile(session_dir, sprintf( ...
        'b1_double_peak_lorentz_tracking_ridge_guided_failure_retry_%s_upper_jump_gamma_R2.csv', ...
        variant.id)));
...
for i = 1:size(copies, 1)
    ...
    if isfile(src)
        dst = fullfile(session_dir, sprintf( ...
            'b1_double_peak_lorentz_tracking_ridge_guided_failure_retry_%s_%s', ...
...
path = fullfile(summary_dir, ...
    'RIDGE_GUIDED_FAILURE_RETRY_METRIC_GATE_PASSED_NO_FIT.txt');
...
writetable(anchor, fullfile(summary_dir, ...
    'b1_double_peak_manual_anchor_template_failure_retry.csv'));
end
...
fid = fopen(path, 'w', 'n', 'UTF-8');
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, '# B1 Lorentz ridge-guided failure retry\n\n');
...
fprintf(fid, '- Retry mechanism: keep all fitted Lorentz components, including very weak second peaks.\n');
...
targets = {summary_dir};
names = {'60_lorentz_tracking_ridge_guided_failure_retry_summary'};
if register_best
    sessions = {'590', 'n0', '20w'};
    for si = 1:numel(sessions)
        ...
        names{end + 1} = sprintf( ...
            '%02d_%s_lorentz_tracking_ridge_guided_failure_retry_best_extraction', ...
            60 + si, sessions{si}); %#ok<AGROW>
    end
end
...
block = sprintf(['<!-- B1_FAILURE_RETRY_260510_START -->\n', ...
...
pattern = '<!-- B1_FAILURE_RETRY_260510_START -->[\s\S]*?<!-- B1_FAILURE_RETRY_260510_END -->';
