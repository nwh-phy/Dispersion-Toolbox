...
function testRapidRisePlateauPathKeepsSmallQAnchorAndAllowsFastRise(testCase)
q = [0.005; 0.010; 0.015; 0.020; 0.030; 0.045; 0.065; 0.085];
n = numel(q);
candidate_id = (1:(2*n)).';
q_rep = repelem(q, 2);
source = repmat({'rapidrise_plateau'; 'high_energy_flat'}, n, 1);
lower = repelem([380; 430; 520; 600; 650; 680; 700; 710], 2);
upper = repelem([455; 660; 960; 1190; 1270; 1310; 1330; 1340], 2);
r2 = repmat([0.95; 0.995], n, 1);
gamma = repmat(90, 2*n, 1);

flat_rows = 2:2:(2*n);
lower(flat_rows) = 720;
upper(flat_rows) = 1260 + (1:n).' .* 4;
...
selection = b1_double_peak_candidate_path_select(candidates, ...
    sessionKey="no_PL2_20w_2film", ...
    upperTrendMode="rapidrise_plateau", ...
    upperTrendAnchorQAbsAinv=0.005, ...
    upperTrendSmallQAbsAinv=0.02, ...
    upperTrendPlateauQAbsAinv=0.06);

selected = selection.path_selection;
verifyEqual(testCase, selected.candidate_source{1}, ...
    'rapidrise_plateau');
verifyGreaterThan(testCase, selected.upper_energy_meV(4) - ...
    selected.upper_energy_meV(1), 700);
verifyEqual(testCase, selected.upper_trend_mode{1}, ...
    'rapidrise_plateau');
verifyTrue(testCase, any(strcmp(selected.upper_trend_region, ...
    'small_q_rapidrise')));
end


function testRapidRisePlateauPathRejectsIsolatedUpperSpike(testCase)
q = [0.005; 0.010; 0.015; 0.020; 0.030; 0.045; 0.065];
n = numel(q);
candidate_id = (1:(2*n)).';
q_rep = repelem(q, 2);
source = repmat({'continuous_rise'; 'isolated_spike'}, n, 1);
lower = repelem([380; 430; 520; 600; 650; 680; 700], 2);
upper = repelem([455; 640; 850; 1030; 1180; 1260; 1300], 2);
r2 = repmat([0.95; 0.999], n, 1);
gamma = repmat(90, 2*n, 1);

spike_rows = 2:2:(2*n);
lower(spike_rows) = lower(spike_rows - 1);
upper(spike_rows) = upper(spike_rows - 1);
upper(2*4) = 1260;
upper(2*5) = 1040;
...
selected = selection.path_selection;
verifyEqual(testCase, selected.candidate_source{4}, ...
    'continuous_rise');
verifyLessThan(testCase, max(abs(diff(selected.upper_energy_meV))), 260);
verifyTrue(testCase, ismember('upper_trend_penalty_meV', ...
...
function testUpperRapidRisePlateauScriptDeclaresNoFitWorkflow(testCase)
script_path = fullfile(testCase.TestData.project_root, ...
    'case_studies', 'bisb2026', 'scripts', ...
    'run_b1_lorentz_tracking_upper_rapidrise_plateau.m');
...
required = { ...
    ...
    'upper_trend_diagnostics.csv', ...
    'no physical fit was run'};
for i = 1:numel(required)
    verifyNotEmpty(testCase, strfind(src, required{i}), ...
        sprintf('Missing upper rapidrise token: %s', required{i}));
end
verifyEmpty(testCase, strfind(src, 'DEPRECATED_B1_500MEV_WINDOW'));
verifyEmpty(testCase, strfind(src, 'run_b1_physical_fit_analysis'));
end
